...
class FirebaseServices {
  final CollectionReference _chatContentItemCollection =
      FirebaseFirestore.instance.collection(
          "Chat Content Items Collection Ref"); //Firebase Firestore col ref for all messages

  ///function to get chat app (me) data from firestore.
  Future<User?> fetchMyData() async {
    final DocumentSnapshot userDoc =
        await FirebaseFirestore.instance.doc("My Doc Ref").get();
    if (userDoc.exists) {
  ...
  Future<User?> fetchRecipientData() async {
    final DocumentSnapshot userDoc =
        await FirebaseFirestore.instance.doc("Recipient Doc Ref").get();
    if (userDoc.exists) {
      return User(id: userDoc.id, name: userDoc[FieldNames.nameField]);
    }
    return null;
