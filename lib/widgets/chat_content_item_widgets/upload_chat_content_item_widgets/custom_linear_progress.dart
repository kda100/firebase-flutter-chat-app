...
class CustomLinearProgress extends StatefulWidget {
  ...
  CustomLinearProgress({required this.uploadTask});
...
class _CustomLinearProgressState extends State<CustomLinearProgress> {
  ...
  @override
  void initState() {
    uploadFileTaskStream =
        widget.uploadTask.snapshotEvents.listen((uploadFileTaskSnapshot) {
      setState(() {
        progress = uploadFileTaskSnapshot.bytesTransferred.toDouble() /
            uploadFileTaskSnapshot.totalBytes.toDouble();
      });
    });
    super.initState();
  }
